...
HOST_COUNT=1
MAC_ADDRESS_BASE="0x1f00"
PLATFORM_COMPACT_NAME="com.mitac.Hardware.Chassis.Model.Capri_V2"
...
action_keep_minimum_cooling () {
        mapfile array < <(find /sys/devices/platform/pwm-fan? -name "pwm1")
        for i in "${!array[@]}"
        do
                pwm_dev=${array[i]}
                echo "Set PWM to $pwm_dev after transit to powered off"
                /bin/sh -c "echo 15 > $pwm_dev"
        done
}
